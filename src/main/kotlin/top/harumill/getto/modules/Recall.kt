...
import top.harumill.getto.tools.MessagesPool
...
class Recall(
    ...
    override suspend fun onEnable() {
        ...
        gettoEventChannel.filter { it is FriendMessageEvent && it.friend.id == Getto.info.administrator }.subscribeFriendMessages {
            "Recall" { target ->
                when(target) {
                    "on" -> {
                        switch = true
